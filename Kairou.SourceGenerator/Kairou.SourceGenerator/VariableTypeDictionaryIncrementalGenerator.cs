...
public class VariableTypeDictionaryIncrementalGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        var typeConverterDeclarations = context.SyntaxProvider
            ...
            .Select(static (tuple, ct) =>
            {
                ...
                if (classSymbol.IsSubclassOf(Symbols.VariableType(compilation)) == false) return null;
        ...
        context.RegisterSourceOutput(
            ...
            static (context, tuple) =>
            {
                ...
                ImmutableArray<INamedTypeSymbol> variableTypeTypeSymbols = tuple.Right;
                Emit(context, compilation, variableTypeTypeSymbols);
            });

        static void Emit(SourceProductionContext context, Compilation compilation, ImmutableArray<INamedTypeSymbol> variableTypeTypeSymbols)
        {
            if (variableTypeTypeSymbols.Length == 0) return;

            string typeFullName = Symbols.VariableTypeDictionary(compilation).ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
            ...
            {
                builder.AppendIndentedLine($"static class VariableTypeRegistrar");
                using (new BlockScope(builder))
                {
                    ...
                    using (new BlockScope(builder))
                    {
                        ...
                        foreach (var variableTypeTypeSymbol in variableTypeTypeSymbols)
                        {
                            builder.AppendIndentedLine($"new {variableTypeTypeSymbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}().Register();");
                        }
                    }
                }
            }
